...
public class PRINT {
	...
	public void printusermetrics(FileWriter file1,ArrayList<USER>listofuser,Logger logger) throws IOException {
		  ...
		  for(int c=0;c<listofuser.size();c++) {
			 file1.write("{\"userId\":"+listofuser.get(c).getUserId()+","+"\"totalNumberofInstance\":"+listofuser.get(c).getTotalNumberofýnstance()+", \"Number of unique instance\":"+
			...
			logger.info("user id: "+listofuser.get(c).getUserId()+" Number of dataset is calculated : "+listofuser.get(c).getDataset().size());
			logger.info("user id: "+listofuser.get(c).getUserId()+" Total number of Instance is calculated: "+listofuser.get(c).getTotalNumberofýnstance());
